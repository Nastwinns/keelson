...
use haw_merge::git::GitMerge;
use haw_merge::{self, MergeError, Side};
...
fn slices_group_by_top_level_component() {
    ...
    let slices = haw_merge::slice_conflicts(&paths);
...
fn integration_branch_name_is_sanitized() {
    assert_eq!(
        haw_merge::integration_branch("release/2.x"),
        "haw/merge/release-2.x"
    );
}
...
fn plan_slices_the_conflicts() {
    ...
    let state = tmp.path().join(".haw");

    let plan = haw_merge::plan(&GitMerge, &repo, &state, "repo", "feature", None).unwrap();

    assert_eq!(plan.source, "feature");
    assert_eq!(plan.target, "main");
    assert_eq!(plan.integration, "haw/merge/feature");
...
fn resolve_each_slice_then_cleanup_lands_one_merge_commit() {
    ...
    haw_merge::plan(&GitMerge, &repo, &state, "repo", "feature", None).unwrap();

    // Take theirs for src, ours for docs.
    let after_src =
        haw_merge::resolve(&GitMerge, &repo, &state, "repo", "src", Some(Side::Theirs)).unwrap();
    assert!(
        after_src
            .slices
            .iter()
    ...
    let err = haw_merge::cleanup(&GitMerge, &repo, &state, "repo", None).unwrap_err();
    assert!(matches!(err, MergeError::Unresolved(ref v) if v == &vec!["docs".to_string()]));

    haw_merge::resolve(&GitMerge, &repo, &state, "repo", "docs", Some(Side::Ours)).unwrap();

    let report =
        haw_merge::cleanup(&GitMerge, &repo, &state, "repo", Some("merge feature")).unwrap();
    ...
    assert!(!branch_exists(&repo, "haw/merge/feature"));
    assert!(haw_merge::load_plan(&state, "repo").unwrap().is_none());
}
...
fn abort_restores_main_and_clears_state() {
    ...
    haw_merge::plan(&GitMerge, &repo, &state, "repo", "feature", None).unwrap();
    assert_eq!(GitMerge_branch(&repo), "haw/merge/feature");

    haw_merge::abort(&GitMerge, &repo, &state, "repo").unwrap();
...
fn plan_refused_when_already_planned() {
    ...
    haw_merge::plan(&GitMerge, &repo, &state, "repo", "feature", None).unwrap();
    let err = haw_merge::plan(&GitMerge, &repo, &state, "repo", "feature", None).unwrap_err();
...
fn plan_rejects_already_merged_source() {
    ...
    let err = haw_merge::plan(&GitMerge, &repo, &state, "repo", "stale", None).unwrap_err();
    ...
    assert!(!branch_exists(&repo, "haw/merge/stale"));
}
...
/// `git init` plus a repo-local identity, so haw's own git invocations
