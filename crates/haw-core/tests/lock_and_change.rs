...
use haw_core::git::RevKind;
use haw_core::lock::{LOCK_VERSION, LockError, LockedRepo, Lockfile};
use haw_core::manifest::Manifest;
use haw_core::resolver;
use haw_core::workspace::branch_for;
...
fn lockfile_round_trips() {
    ...
    let path = dir.path().join("haw.lock");
...
fn branch_policy_never_detaches() {
    ...
    assert_eq!(branch_for("v6.1.2", RevKind::Tag), "haw/v6.1.2");
    assert_eq!(branch_for("release/2.x", RevKind::Tag), "haw/release-2.x");
    let sha = "b".repeat(40);
    assert_eq!(branch_for(&sha, RevKind::Sha), format!("haw/{sha}"));
}
...
mod pin_tests {
    ...
    fn open_manifest_honors_custom_filename_and_root() {
        ...
        // lock/state live beside the manifest, not next to a hardcoded haw.toml
        assert_eq!(ws.lock_path(), dir.path().join("haw.lock"));
...
mod edit_tests {
    ...
    use haw_core::manifest::edit::{self, EditError, NewRepo};
